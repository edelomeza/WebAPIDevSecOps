# Pruebas Unitarias — TipoEmpleado (GET)

## Archivos involucrados

| Archivo | Tipo | Tests |
|---|---|---|
| `UnitTest/TipoEmpleado/GetTests.cs` | Tests | 8 |

**Total: 8 tests — 8/8 correctos**

---

## GET /api/v1.0/tipoempleado (`GetAll`) — 4 tests
...
| `GetAll_ReturnsItemsWithDefaultPagination` | 2 tipos | TotalCount=2, Items.Count=2 |
| `GetAll_WithCustomPageSize_ReturnsCorrectSize` | 5 tipos, pageSize=2 | Items.Count=2, PageSize=2 |
| `GetAll_NullQueryParams_UsesDefaults` | QueryParams=null | PageNumber=1, PageSize=20 |

## GET /api/v1.0/tipoempleado/{id} (`GetById`) — 4 tests
...
dotnet test UnitTest --filter "FullyQualifiedName~TipoEmpleado"       → 8/8
```

## Patrón utilizado

1. `DbContextMock.GetDbContext()` — BD InMemory única por test
2. Seed de datos con `context.EmpCatTipoEmpleado.Add`
3. Controller creado via helper: `new TipoEmpleadoController(new TipoEmpleadoService(context))`
4. Assert con FluentAssertions
5. Sin hasher ni DbResilience — TipoEmpleado es solo lectura (solo GetAll + GetById)
